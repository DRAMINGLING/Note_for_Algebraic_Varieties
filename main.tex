...
\end{remark}
\begin{proposition}
    Let $L = (\{U_{r}\}, g_{rl})$ be a line bundle over $X$.
    Assume $s_{0}, \cdots, s_{n}\in \Gamma(X, L)$, where $s_{i} = \{s_{ir}\}$
...
\begin{proof}
    ...
    And $g_{rl}s_{jl} = s_{jr} = \frac{s_{jr}}{s_{ir}} \cdot s_{ir} = (\frac{x_{j}}{x_{i}} \circ \phi_{L}) \circ s_{ir}$, done!
\end{proof}
\begin{example}
