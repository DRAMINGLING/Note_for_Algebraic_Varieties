...
\begin{proof}
    ...
    Note that each fiber of $\varphi$ is isomorphic to $\mathbb{A}_{k}^{1}$, by Corollary \ref{Corollary 4.8}, 
    $\dim X = \dim Y + 1 = \dim_{k} T_{\beta}Y + 1 = \dim_{k} T_{\alpha}X$ so that $X$ is smooth at $\alpha$.
    In addition, the following matrix has rank 
    \begin{equation}
        \begin{pmatrix}
            M & 0 \\
