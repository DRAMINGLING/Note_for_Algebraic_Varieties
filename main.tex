...
\begin{theorem}[\textbf{\emph{Krull's Principal Ideal Theorem}}]
    ...
    Then all prime ideals in $V(f)$ are of height $1$.
    \label{Theorem 4.2}
\end{theorem}
\begin{remark}
    For noetherian ring $A$ and $f\in A$, we also has Krull's Principal Ideal Theorem that all prime ideals in $V(f)$ are of height at most $1$ and take equality if $f$ is not zero-divisor.
\end{remark}
\begin{theorem}
...
\begin{proof}
    ...
    By Theorem \ref{Theorem 4.4}, $\dim Y = \dim X = n$.
...
\begin{proof}
    ...
    Thus by Theorem \ref{Theorem 4.4}, $\dim X \times Y = \dim X + \dim Y$.
\end{proof}
\begin{corollary}
...
\begin{proof}
    ...
    By similar argument as proof of Theorem \ref{Theorem 4.4}, we replace $X$ by graph $\Gamma_{f}$ of $f$ and there is a commutative diagram
    \begin{equation}
        \begin{tikzcd}
            \mathbb{A}_{k}^{n + m} \arrow[r] &
            \mathbb{A}_{k}^{n - 1 + m} \arrow[r] &
...
\end{remark}
\begin{lemma}
    Let $X$ be a variety, $Z$ irreducible subvariety of $X$ of codimension $r$.
