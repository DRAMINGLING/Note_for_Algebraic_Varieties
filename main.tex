...
\begin{proposition}
    ...
    Define a rational map $\phi_{L}: X \dashrightarrow \mathbb{P}_{k}^{n}$ by gluing up $U_{k} \dashrightarrow \mathbb{P}_{k}^{n}$ sending $x$ to $[s_{0k}(x), \cdots, s_{nk}(x)]$.
...
\end{proposition}
\begin{proof}
    Clearly, $\phi_{L}$ is a morphism, want to show that $(\{\phi_{L}^{-1}(D_{+}(x_{i}))\}, \frac{x_{j}}{x_{i}} \circ \phi_{L}) \sim (\{U_{k}\}, g_{kl})$.
    In fact, for all $x\in U_{k} \cap \phi_{L}^{-1}(D_{+}(x_{i}))$, by definition, $s_{ik}(x) \neq 0$.
    Hence $s_{ik}$ is an invertible regular function on $U_{k} \cap \phi_{L}^{-1}(D_{+}(x_{i}))$.
    And $g_{kl}s_{jl} = s_{jk} = \frac{s_{jk}}{s_{ik}} \cdot s_{ik} = (\frac{x_{j}}{x_{i}} \circ \phi_{L}) \circ s_{ik}$, done!
\end{proof}
\begin{example}
